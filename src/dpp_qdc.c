...
int SetSyncMode(int handle, int timing)
{
  ...
  uint32_t reg,d32;
  // 
  //   for(i=0; i< totalNumberOfDigitizers; i++) 
  //   {
...
//   ret = CAEN_DGTZ_ReadRegister(handle, ADDR_GLOBAL_TRG_MASK, &reg);
//   printf("ADDR_GLOBAL_TRG_MASK[] PRE  = 0x%08x\n",reg);
  ...
  globalret |= ret;
//   ret = CAEN_DGTZ_ReadRegister(handle, ADDR_GLOBAL_TRG_MASK, &reg);
//   printf("ADDR_GLOBAL_TRG_MASK[] POST = 0x%08x\n",reg);
